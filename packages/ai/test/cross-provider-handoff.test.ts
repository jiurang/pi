 ...
 * 获取指定 provider 的 API key —— 先检查 OAuth 存储,再检查环境变量
 */
 ...
 * 同步检查 API key 是否可用(仅检查环境变量,供 skipIf 使用)
 */
 ...
 * 检查是否有任意一个 provider 具备可用的 API key(供 describe 层级的 skipIf 使用)
 */
